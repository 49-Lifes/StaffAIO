...
public final class DiscordHook extends Hook {
    @Override
    protected boolean shouldLoad() {
        if (!super.shouldLoad()) return false;
        return getPlugin().getSettings().getBoolean("hooks.discord.enabled");
    }
    ...
    public void sendStaffChatMessage(User player, String message) {
        final Optional<PlaceholderAPIHook> papiHook = getPlugin().getHookManager().getHook(PlaceholderAPIHook.class);
        String rank = "";
        if (papiHook.isPresent()) {
            rank = papiHook.get().getUserPrefix(player);
        }
        final DiscordWebhook.EmbedObject.EmbedObjectBuilder embed = DiscordWebhook.EmbedObject.builder()
                .title("StaffChat || " + Text.removeColorCodes(rank) + " " + player.getUsername())
                .description(message)
                .thumbnail(new DiscordWebhook.EmbedObject.Thumbnail(getSkinURL(player)));

        final DiscordWebhook webhook = new DiscordWebhook(getPlugin().getSettings().getString("hooks.discord.webhook"));
    ...
    public void sendStaffChatJoin(User player) {
        final Optional<PlaceholderAPIHook> papiHook = getPlugin().getHookManager().getHook(PlaceholderAPIHook.class);
        String rank = "";
        if (papiHook.isPresent()) {
            rank = papiHook.get().getUserPrefix(player);
        ...
        final DiscordWebhook.EmbedObject.EmbedObjectBuilder embed = DiscordWebhook.EmbedObject.builder()
                .title("StaffChat || " + Text.removeColorCodes(rank) + " " + player.getUsername() + " has joined the server!")
                .thumbnail(new DiscordWebhook.EmbedObject.Thumbnail(getSkinURL(player)));
    ...
    public void sendStaffChatLeave(User player) {
        final Optional<PlaceholderAPIHook> papiHook = getPlugin().getHookManager().getHook(PlaceholderAPIHook.class);
        String rank = "";
        if (papiHook.isPresent()) {
            rank = papiHook.get().getUserPrefix(player);
        ...
        final DiscordWebhook.EmbedObject.EmbedObjectBuilder embed = DiscordWebhook.EmbedObject.builder()
                .title("StaffChat || " + Text.removeColorCodes(rank) + " " + player.getUsername() + " has left the server!")
                .thumbnail(new DiscordWebhook.EmbedObject.Thumbnail(getSkinURL(player)));
    ...
    private String getSkinURL(User player) {
        return getPlugin().getSettings().getString("hooks.discord.skin-api")
